...
def generate_vocabulary(sc, corpusRDD, min_count=10):
	wordCount = corpusRDD.flatMap(lambda list_of_words: list_of_words).map(lambda x: (x, 1)).reduceByKey(add)
	filteredVocab = wordCount.filter(lambda (word, count): count > min_count)
	vocab = filteredVocab.map(lambda (word, count): word).collect()
	return vocab
